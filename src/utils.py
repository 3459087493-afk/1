...
try:
    ...
    class _FallbackLightningModule(torch.nn.Module):
        def log(self, *args, **kwargs):
            pass

        def save_hyperparameters(self, ignore=None, *args, **kwargs):
            ignore = set(ignore or [])
            frame = sys._getframe(1)
            values = {
                key: value
                for key, value in frame.f_locals.items()
                if key not in ignore and key not in {"self", "__class__"}
            }
...
def prepare_dataset_for_evaluation(
    ...
    if dataset_name == "KPI":
        ...
        df = df.rename(columns={"tf_mid": "value", "outlier": "target"})
