...
for name, dd in [('full', d_full), ('eval200', d_200), ('sanity', d_s1)]:
    ...
    acc = (rp == dd['y']).mean()
    print(f'  {name}: ref_acc={acc:.6f} shape={ref_logits.shape}')
...
ref1 = d_s1['ref_logits']; x0 = d_s1['X'][0]
hf, hi = kernel(x0)
...
rows = [
    ...
    ('notes','Q16 S0. LUT=14004 DSP=156 hw_acc=%.4f latency=%.2fus.' % (hc/na, times2.mean())),
]
...
print(f'  {CODE}_per_sample_outputs.npz')

print()
print('='*60)
print('Q16 S0 COMPLETE: acc=%.4f F1=%.4f latency=%.2fus speedup=%.1fx' % (hc/na, f1, times2.mean(), speedup_fast))
print('='*60)
