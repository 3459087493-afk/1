...
for i in range(100):
    ...
    ip.write(CTL, 0x1)
    while (ip.read(CTL) & 0x2) == 0: pass
    t1_l = time.perf_counter()
    times[i] = (t1_l-t0_l)*1e6
...
for i in range(1000):
    ...
    while (ip.read(CTL) & 0x2) == 0: pass
    t1_l = time.perf_counter()
    times2[i] = (t1_l-t0_l)*1e6

speedup_fast = 810000.0 / times2.mean()
speedup_rec = 1480000.0 / times2.mean()

print(f'\nResults: acc={hc/na:.4f} F1={f1:.4f} latency={times2.mean():.2f}us speedup={speedup_fast:.1f}x')
...
rows = [
    ...
    ('notes','Q16 S0. LUT=14004 DSP=156 hw_acc=%.4f F1=%.4f latency=%.2fus.' % (hc/na, f1, times2.mean())),
]
...
print(f'Saved {CODE}_pynq_result.csv and per_sample files')
