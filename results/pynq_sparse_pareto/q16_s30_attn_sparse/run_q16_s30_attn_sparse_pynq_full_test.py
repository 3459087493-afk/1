#!/usr/bin/env python3
"""
Ours-Q16-S30-AttnSparse PYNQ-Z2 Full-Test
=============================================
Validate Ours-Q16-S30-AttnSparse (d=16, ff=32, ACTIVE_DIM=11) on full IMG_DS 3444-sample test set.

I/O: ap_fixed<16,6> = int16 over m_axi DMA (same as small_B verified configuration).

Stages:
  0. Reference sanity check (no hardware)
...
Run on PYNQ-Z2:
    sudo python3 run_q16_s30_attn_sparse_pynq_full_test.py
"""

import os, sys, time, csv, numpy as np

# ------------------------------------------------------------------
# ap_fixed<16,6> fixed-point conversion (Q6.10, scale=1024)
# ------------------------------------------------------------------
FP_SCALE      = 1 << 10   # 1024
FP_INT16_MAX  =  32767
FP_INT16_MIN  = -32768
...
# ------------------------------------------------------------------
# Stage 0: Reference sanity check (no hardware)
# ------------------------------------------------------------------
def reference_sanity_check():
    ...
    print("=" * 60)
    expected = {
        "full 3444": ("q16_s30_attn_sparse_test_3444.npz", 0.9419),
        "eval 200":  ("q16_s30_attn_sparse_eval200.npz",  0.9500),
        "sanity 1":  ("q16_s30_attn_sparse_sanity.npz",    None),
    }
    for name, (path, exp) in expected.items():
        ...
        if exp is not None and abs(ref_acc - exp) > 0.01:
...
# ---- Paths ----
BIT_FILE    = "q16_s30_attn_sparse.bit"
HWH_FILE    = "q16_s30_attn_sparse.hwh"
EVAL_200    = "q16_s30_attn_sparse_eval200.npz"
FULL_TEST   = "q16_s30_attn_sparse_test_3444.npz"
SANITY      = "q16_s30_attn_sparse_sanity.npz"
OUTPUT_CSV  = "q16_s30_attn_sparse_pynq_result.csv"

N_WARMUP = 10

# ---- AXI-Lite register offsets (from ximgds_linear_dense_q16_hw.h) ----
S_AXI_BASE   = 0x40000000
CONTROL_ADDR = 0x00
INPUT_R_ADDR = 0x10   # input_r[31:0]; 0x14 = input_r[63:32]
LOGITS_ADDR  = 0x1C   # logits[31:0];  0x20 = logits[63:32]

# ---- Ours-Q16-S30-AttnSparse constants ----
SEQ_LEN      = 16
INPUT_DIM    = 64
N_CLASSES    = 2
...
def main():
    ...
    print("=" * 60)
    print("Ours-Q16-S30-AttnSparse PYNQ-Z2 FULL-TEST")
    print("d_model=16  ff_dim=32  ACTIVE_DIM=11  sparsity=31.25%")
    print("Vivado LUT=TBD  DSP=TBD  SW acc=0.9521")
    print("=" * 60)
    print(f"\n  I/O: {INPUT_ELEMS} int16 input, {LOGITS_ELEMS} int16 output")
    ...
    rows = [
        ("model_id", "Ours-Q16-S30-AttnSparse"), ("d_model", 16), ("ff_dim", 32), ("quant_bits", 16),
        ("vivado_lut", TBD), ("vivado_dsp", TBD), ("vivado_ff", TBD), ("vivado_bram", TBD),
        ("software_accuracy", 0.9521),
        ("io_dtype", "int16 (ap_fixed<16,6>)"), ("fp_scale", FP_SCALE),
        ("n_correctness_samples", 3444),
        ("prediction_match_rate_vs_Q16_ref", round(mr3, 6)),
        ("hw_label_accuracy", round(ha3, 6)),
        ("ref_label_accuracy", round(ra3, 6)),
        ("max_abs_error_vs_Q16_ref", round(mx3, 8) if np.isfinite(mx3) else "nan"),
        ("mean_abs_error_vs_Q16_ref", round(mn3, 8) if np.isfinite(mn3) else "nan"),
        ...
        ("pynq_measured", "True"),
        ("speedup_vs_ViT4Mal_fastest", round(810000.0 / km1k, 2)),
        ("speedup_vs_ViT4Mal_recommended", round(1480000.0 / km1k, 2)),
        ("notes", f"Ours-Q16-S30-AttnSparse (d=16/ff=32) PYNQ-Z2 full-test ({n} samples). "
                  f"ref_acc={ra3:.4f}. HW acc={ha3:.4f}. Match={mr3:.4f}. "
                  f"latency={km1k:.2f}us. int16 I/O (ap_fixed<16,6>)."),
    ]
