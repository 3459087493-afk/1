...
ours = [
    ...
    ("Q16-S0",    14004, 156, 12903, 15, 94.13, 96.53, 97.87, 95.22, 19.761, 24.837,  0.00, 40.99),
    ("Q16-S10",   14035, 156, 12945, 15, 94.11, 96.51, 97.80, 95.25, 20.288, 26.487, 12.50, 39.92),
    ("Q16-S20",   14041, 156, 12945, 15, 94.22, 96.59, 97.68, 95.52, 21.333, 24.596, 18.75, 37.97),
    ("Q16-S30",   14044, 156, 12945, 15, 94.13, 96.53, 97.74, 95.35, 19.012, 22.935, 31.25, 42.60),
    ("Q8-S0",     14019, 124, 13373, 15, 90.97, 94.50, 98.60, 90.74, 18.897, 21.791,  0.00, 42.86),
    ("Q8-S30",    14031, 124, 13395, 15, 91.35, 94.76, 98.43, 91.35, 19.250, 23.126, 31.25, 42.08),
]
...
def fig1():
    ...
    # Annotations
    annotate_point(ax, ours[3][1], ours[3][4]+0.12, "Q16-S30", 12, 10, color=C_OURS_Q16)
    annotate_point(ax, ours[2][1], ours[2][4]+0.05, "Q16-S20\n(best acc.)", 16, 4, color=C_OURS_Q16)
    annotate_point(ax, ours[4][1], ours[4][4]-0.18, "Q8-S0\n(lowest LUT)", -10, -16, color=C_OURS_Q8)
...
def fig3():
    ...
    # Annotate Q16-S30
    ax_a.annotate("Q16-S30: same acc as Q16-S0\nbut higher sparsity",
    ...
    ax_b.annotate("Q16-S30: lower latency\nthan Q16-S0 at same accuracy",
