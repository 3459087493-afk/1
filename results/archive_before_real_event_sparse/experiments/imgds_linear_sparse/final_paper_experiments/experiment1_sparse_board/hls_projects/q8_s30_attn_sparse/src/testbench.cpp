...
int main() {
        ...
        for (int s = 0; s < SEQ_LEN; s++)
            for (int d = 0; d < INPUT_DIM; d++)
                input[s][d] = data_t(eval_inputs_flat[n * SEQ_LEN * INPUT_DIM
                                                      + s * INPUT_DIM + d]);
        data_t logits[2];
        imgds_linear_dense_q8(input, logits);
    ...
    printf("\n");
    printf("==================================================\n");
    printf("Q8-S0  CSIM Results (N=%d)\n", N_EVAL_SAMPLES);
    printf("==================================================\n");
    printf("  Prediction match rate vs Q8 ref:  %.4f  (%d/%d)\n",
    ...
    printf("  Max  abs error vs Q8 ref:         %.8f\n", max_err);
    printf("  Mean abs error vs Q8 ref:         %.8f\n", mean_err);
    ...
    bool relaxed_pass = (match_rate >= 0.98f) && (hw_accuracy >= 0.90f);
    bool match_ok     = (match_rate >= 0.95f);

    printf("\n==================================================\n");
    printf("  Match rate >= 0.95:  %s\n", match_ok     ? "PASS" : "FAIL");
    printf("  Relaxed criteria  :  %s\n", relaxed_pass ? "PASS" : "FAIL");
    printf("==================================================\n");

    if (match_ok)
        printf("\nCSIM PASSED. HW accuracy=%.4f.\n", hw_accuracy);
    else
        printf("\nCSIM FAILED (match rate %.4f < 0.95).\n", match_rate);
