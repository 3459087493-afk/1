...
typedef struct {
    ...
    XQ16_esla_k13_uio_map maps[ MAX_UIO_MAPS ];
} XQ16_esla_k13_uio_info;

/***************** Variable Definitions **************************************/
static XQ16_esla_k13_uio_info uio_info;
...
static int uio_info_read_name(XQ16_esla_k13_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XQ16_esla_k13_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XQ16_esla_k13_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XQ16_esla_k13_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XQ16_esla_k13_Initialize(XQ16_esla_k13 *InstancePtr, const char* InstanceName) {
	XQ16_esla_k13_uio_info *InfoPtr = &uio_info;
