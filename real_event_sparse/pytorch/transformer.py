...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, x, mask=None, return_attention=False):
        ...
        values, attention = scaled_dot_product(q, k, v, mask=mask)  # [Batch, Head, SeqLen, Dims]
...
class TransformerAnomalyDetector(MetricsMixin):
    def __init__(
        self,
        input_dim,
        block_input_dim,
...
class LinearRegressionAnomalyDetector(MetricsMixin):
    def __init__(
        self,
        input_dim,
        learning_rate,
...
class LinearTransformerAnomalyDetector(MetricsMixin):
    def __init__(
        self,
        input_dim,
        block_input_dim,
...
class _UNSWLinearAttention(nn.Module):
    """Kernelized self-attention with linear sequence-length complexity."""
    ...
    def forward(self, x):
        ...
        # Reorder Q(K^T V) so no sequence-by-sequence attention matrix is built.
        key_value = torch.matmul(key.transpose(-2, -1), value)
        key_sum = key.sum(dim=1, keepdim=False).unsqueeze(-1)
        normalizer = torch.matmul(query, key_sum).clamp_min(self.eps)
        attended = torch.matmul(query, key_value) / normalizer
        return self.output(attended)


class _UNSWLinearEncoderBlock(nn.Module):
    def __init__(self, d_model, dim_feedforward, dropout):
        super().__init__()
        self.attention = _UNSWLinearAttention(d_model)
...
class LinearUNSWAnomalyDetector(nn.Module):
    """Linear Transformer classifier for preprocessed UNSW-NB15 samples."""
    ...
    def forward(self, x):
        ...
        if x.size(1) != self.seq_len or x.size(2) != self.input_dim:
            raise ValueError(
                f"Expected sequence shape ({self.seq_len}, {self.input_dim}), "
