...
void unsw_linear_transformer(
    ...
    for (int sequence = 0; sequence < UNSW_SEQ_LEN; ++sequence) {
        for (int active_feature = 0; active_feature < UNSW_ACTIVE_DIM; ++active_feature) {
            ...
            sparse_key[sequence][active_feature] = data_t(key[sequence][channel]);
            sparse_query[sequence][active_feature] = Q_ACTIVE_MASK[channel]
                ? data_t(query[sequence][channel]) : data_t(0.0f);
        }
    }
    ...
    for (int active_feature = 0; active_feature < UNSW_ACTIVE_DIM; ++active_feature) {
        for (int value_feature = 0; value_feature < UNSW_D_MODEL; ++value_feature) {
            attn_acc_t sum = attn_acc_t(0.0f);
            for (int sequence = 0; sequence < UNSW_SEQ_LEN; ++sequence) {
                sum += attn_acc_t(sparse_key[sequence][active_feature])
    ...
    for (int sequence = 0; sequence < UNSW_SEQ_LEN; ++sequence) {
        attn_acc_t denominator = attn_acc_t(0.0f);
        for (int active_feature = 0; active_feature < UNSW_ACTIVE_DIM; ++active_feature) {
            denominator += attn_acc_t(sparse_query[sequence][active_feature])
                * attn_acc_t(key_sum[active_feature]);
        }
        if (denominator < attn_acc_t(1.0e-6f)) {
            denominator = attn_acc_t(1.0e-6f);
        }
        for (int value_feature = 0; value_feature < UNSW_D_MODEL; ++value_feature) {
            attn_acc_t numerator = attn_acc_t(0.0f);
            for (int active_feature = 0; active_feature < UNSW_ACTIVE_DIM; ++active_feature) {
                numerator += attn_acc_t(sparse_query[sequence][active_feature])
                    * attn_acc_t(key_value[active_feature][value_feature]);
            }
