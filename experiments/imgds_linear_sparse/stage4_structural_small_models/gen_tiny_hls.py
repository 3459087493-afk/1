"""Generate tiny (d=6, ff=12) HLS headers + eval samples."""
...
HLS  = f'{STG4}/hls/tiny_q16_d6_ff12'

D, S, I, F, C = 6, 16, 64, 12, 2

ckpt = torch.load(f'{STG4}/checkpoints/tiny_best.pt', map_location='cpu', weights_only=False)
sd = ckpt['state_dict']
...
hdr = ['// tiny HLS params (d={}, ff={})'.format(D,F),
       '#ifndef HLS_PARAMS_TINY_H','#define HLS_PARAMS_TINY_H','',
       '#include <ap_fixed.h>','',
...
for kn, cn in param_map:
    ...
    if flat.size > 0:
        print('  {}: shape={} len={} [{:.4f}, {:.4f}]'.format(kn, list(v.shape), flat.size, flat.min(), flat.max()))
...
hdr.append('#endif\n')
with open(f'{HLS}/hls/src/hls_params_tiny.h','w') as f: f.write('\n'.join(hdr))
print('hls_params_tiny.h: {} lines'.format(len(hdr)))
...
print('tiny eval200 SW acc: {:.2f}%'.format(swa*100))

esh = ['// tiny eval samples','#ifndef EVAL_SAMPLES_TINY_H','#define EVAL_SAMPLES_TINY_H','',
...
with open(f'{HLS}/hls/src/eval_samples_tiny.h','w') as f: f.write('\n'.join(esh))
