/// imgds_linear_small_a.cpp — small_B (d_model=8, ff_dim=16)
/// Structural small Linear Transformer, PARETO2 DSP-shared architecture.
#include "imgds_linear_small_a.h"
#include "hls_params_small_a.h"
#include <cmath>
...
static void layer_norm_small_a(data_t x[D_MODEL],
...
static void linear_attention_small_a(
...
static void feedforward_small_a(
    data_t x[SEQ_LEN][D_MODEL],
...
#pragma HLS PIPELINE II=1
    ...
    for (int s = 0; s < SEQ_LEN; s++) layer_norm_small_a(x[s], layers_0_norm1_weight, layers_0_norm1_bias);
    feedforward_small_a(x, layers_0_feedforward_0_weight, layers_0_feedforward_0_bias,
                        layers_0_feedforward_3_weight, layers_0_feedforward_3_bias);
    for (int s = 0; s < SEQ_LEN; s++) layer_norm_small_a(x[s], layers_0_norm2_weight, layers_0_norm2_bias);
...
#pragma HLS PIPELINE II=1
    ...
    layer_norm_small_a(pooled, output_norm_weight, output_norm_bias);
