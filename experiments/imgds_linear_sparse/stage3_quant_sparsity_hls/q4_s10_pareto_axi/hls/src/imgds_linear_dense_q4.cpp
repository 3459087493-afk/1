/// imgds_linear_dense_q4.cpp — Q4-S10 (Q4 weights + Q16 data, 10% sparse)
...
// ==========================================================================
//  layer_norm_q4  —  D_MODEL=16, fully serial, small
// ==========================================================================
static void layer_norm_q4(
    data_t x[D_MODEL],
...
// ==========================================================================
static void linear_attention_q4(
    data_t x[SEQ_LEN][D_MODEL],
    const ap_fixed<4,2> Wq_flat[D_MODEL * D_MODEL],
    const ap_fixed<4,2> Wk_flat[D_MODEL * D_MODEL],
    const ap_fixed<4,2> Wv_flat[D_MODEL * D_MODEL],
    const ap_fixed<4,2> Wo_flat[D_MODEL * D_MODEL],
    const data_t bo[D_MODEL]
) {
    // DSP allocation — share multipliers across all MAC operations
#pragma HLS ALLOCATION instances=mul limit=64 operation

    // --- Local buffers (no array partition — serial reuse) ---
    data_t Q[SEQ_LEN][D_MODEL];
    data_t K[SEQ_LEN][D_MODEL];
    data_t V[SEQ_LEN][D_MODEL];

    // --- Step A: Compute Q, K, V projections + feature_map on Q, K ---
    // Serial over positions, serial over output dim, serial over input dim
...
#pragma HLS PIPELINE II=1
    ...
    // --- Step B: KV = K^T @ V   → [D_MODEL][D_MODEL] ---
...
#pragma HLS PIPELINE II=1
    ...
    // --- Step C: K_sum = column-wise sum of K → [D_MODEL] ---
...
// ==========================================================================
static void feedforward_q4(
    data_t x[SEQ_LEN][D_MODEL],
    const ap_fixed<4,2> W0_flat[FF_DIM * D_MODEL],
    const data_t b0[FF_DIM],
    const ap_fixed<4,2> W3_flat[D_MODEL * FF_DIM],
...
// PYNQ-friendly AXI interfaces — Q4-S10
...
#pragma HLS PIPELINE II=1
    ...
    // 2b. LayerNorm1
    for (int s = 0; s < SEQ_LEN; s++)
        layer_norm_q4(x[s], layers_0_norm1_weight, layers_0_norm1_bias);
    ...
    // 2d. LayerNorm2
    for (int s = 0; s < SEQ_LEN; s++)
        layer_norm_q4(x[s], layers_0_norm2_weight, layers_0_norm2_bias);
...
#pragma HLS PIPELINE II=1
    ...
    layer_norm_q4(pooled, output_norm_weight, output_norm_bias);
