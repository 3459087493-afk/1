"""Regenerate Q4 headers: ONLY 2D weight matrices use ap_fixed<4,2>.
1D biases and norm params stay as ap_fixed<16,6> (data_t)."""
...
# Q4 quantize ONLY 2D weight matrices
def q4_quant(w):
    s = 4; mx = 1.75; mn = -2.0
    return torch.clamp(torch.round(w * s), mn * s, mx * s) / s

# Q16 quantize for biases, norms, and 1D params
def q16_quant(w):
    s = 2**10  # ap_fixed<16,6> has 10 fractional bits
    mx = 2**5 - 2**(-10); mn = -2**5
    return torch.clamp(torch.round(w * s), mn * s, mx * s) / s
...
q16_sd = OrderedDict()
for k, v in sd.items():
    iw = 'weight' in k and v.ndim >= 2  # only 2D weight matrices
    is_pos = 'position' in k.lower()
    if iw and not is_pos:
        q4_sd[k] = q4_quant(v)
    elif is_pos:
        # position_embedding stays as higher precision (shape [1,16,16] → flatten)
        q4_sd[k] = v.clone()  # keep as float, write as data_t later
    else:
        q4_sd[k] = v.clone()
...
for k, v in q4_sd.items():
    ...
    elif 'attention' in k and 'weight' in k and sh == (D_MODEL, D_MODEL): vn *= (mask[:,None]*mask[None,:])
...
# active_channels.h (unchanged)
cl = ', '.join(str(c) for c in active_ch)
with open(f'{STG3}/hls/src/active_channels.h','w') as f:
    f.write(f'// Q4-S10 active channels (global pruning, 10% sparse)\n')
...
# hls_params_q4.h — mixed types
# 2D weight matrices → ap_fixed<4,2>
# biases, norms, position_embedding → ap_fixed<16,6> (data_t)
# keys ending with .weight but ndim>=2 are Q4; others are Q16
param_map = [
    ('classifier.bias', 'classifier_bias', 'data_t'),
    ('classifier.weight', 'classifier_weight', 'ap_fixed<4,2>'),
    ('input_projection.bias', 'input_projection_bias', 'data_t'),
    ('input_projection.weight', 'input_projection_weight', 'ap_fixed<4,2>'),
    ('position_embedding', 'position_embedding', 'data_t'),
    ('attention.query.weight', 'layers_0_attention_query_weight', 'ap_fixed<4,2>'),
    ('attention.key.weight', 'layers_0_attention_key_weight', 'ap_fixed<4,2>'),
    ('attention.value.weight', 'layers_0_attention_value_weight', 'ap_fixed<4,2>'),
    ('attention.output.weight', 'layers_0_attention_output_weight', 'ap_fixed<4,2>'),
    ('attention.output.bias', 'layers_0_attention_output_bias', 'data_t'),
    ('norm1.weight', 'layers_0_norm1_weight', 'data_t'),
    ('norm1.bias', 'layers_0_norm1_bias', 'data_t'),
    ('feedforward.0.weight', 'layers_0_feedforward_0_weight', 'ap_fixed<4,2>'),
    ('feedforward.0.bias', 'layers_0_feedforward_0_bias', 'data_t'),
    ('feedforward.3.weight', 'layers_0_feedforward_3_weight', 'ap_fixed<4,2>'),
    ('feedforward.3.bias', 'layers_0_feedforward_3_bias', 'data_t'),
    ('norm2.weight', 'layers_0_norm2_weight', 'data_t'),
    ('norm2.bias', 'layers_0_norm2_bias', 'data_t'),
    ('output_norm.weight', 'output_norm_weight', 'data_t'),
    ('output_norm.bias', 'output_norm_bias', 'data_t'),
]

hdr = ['// Q4-S10 HLS params (mixed: Q4 weights + Q16 biases/norms)',
       '#ifndef HLS_PARAMS_Q4_H','#define HLS_PARAMS_Q4_H','',
       '#include <ap_fixed.h>','','typedef ap_fixed<16,6> data_t;','',
       '#define SEQ_LEN 16','#define INPUT_DIM 64','#define D_MODEL 16',
       '#define FF_DIM 32','#define N_CLASSES 2','#define N_EVAL 200','']

for sfx, cn, dt in param_map:
    found = None
    for k,v in q4_sparse.items():
        if k.endswith(sfx): found = v; break
    if found is None: continue
    flat = found.numpy().flatten()
    hdr.append(f'// {sfx} shape={list(found.shape)} type={dt}')
    hdr.append(f'static const {dt} {cn}[{len(flat)}] = {{')
    hdr.append(fmt_arr(flat)); hdr.append('};'); hdr.append('')
hdr.append('#endif\n')
with open(f'{STG3}/hls/src/hls_params_q4.h','w') as f: f.write('\n'.join(hdr))
print(f'Written: hls_params_q4.h ({len(hdr)} lines, {len(param_map)} params)')

# eval samples — same as before, Q4 S10 works as SW inference
...
print(f'Q4 S10 (mixed precision) SW acc (eval200): {swa*100:.2f}%')

esh = ['// Q4-S10 eval samples','#ifndef EVAL_SAMPLES_Q4_H','#define EVAL_SAMPLES_Q4_H','',
       '#include <ap_fixed.h>','',
